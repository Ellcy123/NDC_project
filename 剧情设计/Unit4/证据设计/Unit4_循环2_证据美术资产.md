...
4. 明确避免统一批号或统一配方栏；本证物不能把七名儿童锁成同一药剂批次。
...
- 小玩法关系：`分析输出`。输入仅为 4118；4213 只提供历史相似特征和死亡编号，不作为同批化验对照。与 4214 共同用于 Whitfield R3：4703证明医院控制下的留样本身有风险，4214再证明它与 Isabel 药瓶属于同一采购记录、同一生产批次和同一配方。
