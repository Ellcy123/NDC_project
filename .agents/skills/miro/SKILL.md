...
Token 和 Board ID 存在 `.Codex/skills/miro/.env`（已被 `.gitignore` 忽略）：
...
**执行**：
```bash
python .Codex/skills/miro/scripts/mindmap_import.py <txt_file> [--root-x 35000] [--root-y 0] [--x-step 350] [--y-step 50]
```
...
```bash
python .Codex/skills/miro/scripts/mindmap_clean.py [--contains "Unit1 大纲"]
```

### 3. `list-mindmap` — 列出 board 上所有 mindmap root

```bash
python .Codex/skills/miro/scripts/mindmap_list.py
```
...
```bash
python .Codex/skills/miro/scripts/mindmap_delete_tree.py <root_id>
```
...
```bash
# 1. Markdown 转 Tab 缩进
python .Codex/skills/miro/scripts/md_to_indent.py \
...
# 3. 推上去
python .Codex/skills/miro/scripts/mindmap_import.py /tmp/outline.txt
