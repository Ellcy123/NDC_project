...
### 4.1.1 NPC Talk 入口配置规则（2026-06-04 v0.3）

`SceneConfig.NPCInfos[].TalkInfo` / `LoopTalkInfo` 必须优先显式挂载对话文件与入口句：

- `id`: 对话链第一句 Talk ID
- `videoScene`: AVG JSON 文件名，不带 `.json`
- `videoEpisode`: 章节，如 `EPI01`
- `videoLoop`: Loop，如 `loop1`
...
配置不一致时必须显式报错，例如 `TalkInfo.id` 指向的 Talk 不属于 `TalkInfo.videoScene`，不能静默改用 NPC 名字猜测。
...
## 修订记录
- 2026-05-28 v0.1：初版，沉淀讨论结论（Ellcy + Codex）
