...
### 疑点 / 指证联动硬规则
改 `DoubtConfig`、`ChapterConfig.doubts`、`ExposeData`、`ChapterConfig.exposes` 时，必须反查：
1. 每个指证出示材料（道具 / 证词）是否至少挂在一个疑点或疑点碎片的 `condition` 里；不存在"只在指证时出现、但没有进入疑点 / 碎片"的游离证据。
2. 该疑点 / 碎片出现 Loop 是否早于或等于首次使用该材料的指证 Loop；不能把当前 Loop 指证要用的证据挂到后续 Loop 才点亮。
3. 查询或汇报疑点配置时，默认列出"疑点 → 触发材料 → 是否指证用"，并标清哪些材料是"指证用"，哪些只是"触发疑点"。
...
### Phase 2 · 落地
1. **基线**：`cd .Codex/skills/config-edit && python validate.py --save .ce_baseline.json`
