...
- 初稿操作员 A 与返修操作员 B 必须分别新建为两个独立 worker；不得用 A 的后续任务承担 B。A、B 调用 Gemini 时也使用独立请求上下文，B 不继承 A 的生成过程或内部讨论。
- 只传本阶段必要的材料、路径、提示词、参数和授权；返修 B 可读取初稿及审核意见，不继承主对话完整历史，也不重复加载无关项目文档。
- 操作员只负责调用 Gemini、返回结果；对白审核由强模型承担。流程为“Sol A 调 Gemini 写作 → 强模型审核 → Sol B 调 Gemini 返修”。这项分派不改变实际调用的 Gemini 模型。
