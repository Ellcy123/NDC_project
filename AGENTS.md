...
NDC 网页端 GPT 生图允许在不同已登录 ChatGPT 账户之间继续执行，账户不同本身不构成暂停理由。原专用对话或结果仍可访问时，接续原 URL、submission 和回执，不得重复提交；若新账户因账户隔离确实缺少旧对话、历史消息或生成结果，可把旧 pending/unknown 以 `account_record_unavailable` 终态封存，保留旧记录及一次真实 attempt 计数，再在新账户新建专用对话，以相同输入／提示词哈希重新生成并计作新的真实 attempt。记录 browser、非敏感账户／浏览器配置标签、切换原因、旧状态、新 URL 和回执；不得记录或搬运认证凭据，不得借此在旧记录可核实时盲目重发或改用其它生图后端。
...
- 每次提交涉及 Skill 的新增、修改或相关功能时，必须检查该 Skill 实际依赖的自有脚本、工具源码、规则、模板、配置示例及依赖清单，将本次新增或修改的必要文件整理到所属项目仓库内，并与相关改动一并提交；不能只提交 `SKILL.md`，把执行所需文件留在个人目录、临时目录或项目外。
- Skill 专用文件优先放在该 Skill 目录内，共用工具放在项目的公共脚本目录；同步修正引用路径，确保其他人拉取仓库后可按文档安装依赖并运行。已有跨仓库唯一主实现的 Skill 继续遵守其归属规则，不复制出第二套主实现。
- 第三方工具记录安装方式、版本和依赖清单；第三方安装包、密钥、虚拟环境、缓存及临时生成产物不纳入 Git。提交前检查必要文件是否被 `.gitignore` 意外忽略，避免漏交。
...
- 当前窗口模型负责读取剧情来源、控制人物知识与信息揭示、写完整初稿，并亲自编写每轮润色提示词；经 NovAI 显式调用 `[次]gemini-3.8-flash`。2026-09-15 用户修订：附完整工作稿、出场人物的性格/情感/已有经历/爱好/关系及 Nyra 全文，不附知识边界清单；要求信息不变，表达更通顺口语化、更符合人设，允许拆句、合句、调整块内接话和适当拌嘴笑点，不限制句数。主模型在本地掌握信息边界，最后核对事实和结构，保留润色后的语言风格。
