...
角色入场景的同一场景必须先完成并冻结全部必需角色／状态的正式生成或合法复用源，再统一进入抠图、去背景、Alpha、注册与合成；不得生成一个角色就先处理一个角色。每个结果回收后只做提取前视觉审核并保存原始下载、来源回执和冻结 SHA-256，继续本场剩余角色。只有完整 scope 逐项无缺失、失败、待回收、结果未知或未冻结项，并登记 `SCENE_CAST_GENERATION_GATE: PASS` 后，才允许本场开始正式角色提取。某角色受阻时可继续其余角色生成，但本场提取保持阻断；不同完整场景可独立推进。旧任务已有提取结果只读保留，从当前节点先补齐剩余生成。
