...
1. **分析任务**：判断复杂度，确定团队配置
2. **确认方案**：告诉用户打算 spawn 哪些角色，等用户说 "go" 再执行
3. **执行**：按下方执行顺序 spawn 团队
4. **汇总**：Lead 汇总所有反馈，输出终稿 + 评分汇总表（格式见 `quality_standards.md` 附录）

### 角色池（按需选取，不是每次全用）

| 角色 | 职责 | 对应 quality_standards.md 模块 | 模型 | 何时选用 |
|------|------|-------------------------------|------|---------|
| **策划** | 生成设计内容 | — | Sonnet | 需要产出新内容时 |
| **评审** | 按质量标准逐项打分、挑硬伤 | 根据内容类型按文档开头速查表选模块 | **Opus** | 几乎所有中等以上任务 |
| **玩家视角** | 模拟玩家走信息获取路径，检验推理体验 | 模块 F（玩家体验）+ A1（破梗检测） | Sonnet | 涉及推理体验、谜题设计时 |
| **信息时序** | 专查跨 Loop 信息泄露、证据权限、疑点解锁 | 模块 G（时序一致性）+ B4（时序符合） | **Opus** | 涉及跨 Loop 联动、State 文件时 |
| **技术可行性** | 评估 Unity 实现难度、配置表可行性、UI 呈现 | 模块 C4（可视化适配） | Sonnet | 涉及新系统机制或 UI 交互时 |

> **模型选择理由**：评审和信息时序需要深度推理发现微妙矛盾，用 Opus；策划和技术可行性用 Sonnet 性价比高；玩家视角用 Sonnet 模拟普通玩家认知水平，太强反而失真。

### 任务分级

| 复杂度 | 判断标准 | 团队配置 | 示例 |
|--------|---------|---------|------|
| **简单** | 改文案、列清单、单文件小修 | 不开 team，直接做 | "把这句台词改通顺" |
| **中等** | 单场景/单 NPC 设计 | 策划 + 评审 | "设计 Loop3 Tommy 的对话" |
| **复杂** | 完整指证设计、多角色场景 | 策划 + 评审 + 玩家视角 | "设计 Loop4 Morrison 指证" |
| **重大** | 多模块联动、整个 Loop 规划 | 策划 + 评审 + 玩家视角 + 信息时序 | "规划 Loop5 整体信息节奏" |

> 技术可行性仅在涉及新系统机制或 UI 交互时加入，纯内容策划不需要。
...
策划 agent → 产出初稿
     ↓
评审 / 玩家视角 / 信息时序（并行）→ 各自独立审查
     ↓
Lead（Opus）汇总反馈 → 输出终稿
```

- **策划先行**：评审类角色必须等策划产出后才开始
- **评审并行**：评审、玩家视角、信息时序互不依赖，同时 spawn
- **评审独立**：评审 agent 只看策划的产出 + `quality_standards.md`，不看策划的思考过程
