...
- 当本 Skill 的授权修图确实需要图像生成时，默认使用 Codex 内置 `image_gen`。只有父任务已按 `ndc-scene-evidence-placement` 明确判定为“在锁定场景或其场景派生容器视图中生成新增承载物、容器或环境叙事物品”等额外物件时，才沿用该父任务的 `chatgpt.com` 网页版生图路线；本 Skill 不得自行扩大网页例外。已有道具放置、非新增物件修复、Big／Icon 和其它局部生成仍使用内置工具。任一路线失败或结果未知时保留调用回执和当前检查点，不跨路线盲目重发。
