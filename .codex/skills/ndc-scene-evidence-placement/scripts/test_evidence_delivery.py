...
class EvidenceDeliveryTests(unittest.TestCase):
    ...
    def command(
        ...
        final: Path,
        output: Path,
        authorization_mask: Path | None = None,
        map_shape_mask: Path | None = None,
    ) -> list[str]:
        command = [
            sys.executable,
            "-B",
        ...
        if map_shape_mask is not None:
            command.extend(["--map-shape-mask", str(map_shape_mask)])
        return command
    ...
    def test_irregular_map_shape_preserves_parent_pixels_and_verifies(self) -> None:
        irregular_final = self.root / "irregular_final.png"
        shape_mask = self.root / "map_shape.png"
        shape = Image.new("L", (64, 64), 0)
        ImageDraw.Draw(shape).polygon(
            [(20, 22), (31, 23), (29, 29), (21, 28)], fill=255
        )
        shape.save(shape_mask)
        irregular = Image.open(self.source).copy()
        ImageDraw.Draw(irregular).polygon(
            [(20, 22), (31, 23), (29, 29), (21, 28)], fill=(180, 40, 25)
        )
        irregular.save(irregular_final)
        output = self.root / "irregular_delivery"

        packaged = subprocess.run(
            self.command(
                irregular_final,
                output,
                authorization_mask=shape_mask,
                map_shape_mask=shape_mask,
            ),
            check=False,
            capture_output=True,
            text=True,
        ...
        manifest_path = output / "delivery_manifest.json"
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        self.assertEqual(manifest["mapCrop"]["shape"], "irregular-alpha")
        with Image.open(output / "SC4025_item_4317.png") as sprite:
            self.assertEqual(sprite.mode, "RGBA")
            self.assertEqual(sprite.getpixel((0, 0)), (180, 40, 25, 255))
            self.assertEqual(sprite.getpixel((11, 7)), (0, 0, 0, 0))
