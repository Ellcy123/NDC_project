...
- Cache one host/state/capability snapshot for the current Photoshop connection and reuse it while the plugin instance and command catalogue stay unchanged; do not re-list the full catalogue for every asset. Before a mutation, re-read only the active document/layer identity and describe any command whose schema or status is not already in the job record.
- When `layer.place_file` is currently `supported`, open the frozen target scene once and place each approved RGBA source from an allowed absolute path directly into that document as a named embedded smart-object layer. Verify the returned layer count, name, kind and embedded status after each placement, then take one whole-scene preview after the placement/transform batch. Do not open each source as a separate document, use clipboard automation, or export a preview after every read-only step.
- `layer.place_file` preserves an accepted source Alpha but does not create or improve a cutout. Prefer native Alpha; otherwise run the approved Select Subject/Remove Background action only on the selected winner, inspect soft and hard edge risk areas, and repair only the failing segments. Never spend Photoshop time isolating both image candidates before selection or tracing the full outline when localized correction is sufficient.
...
- Treat embedded smart objects as placement containers, not as proof of an editable mask or pixel layer. Current placement does not authorize relinking, replacing contents, rasterizing, creating/writing masks, or selection-to-layer operations; use only separately described supported commands and keep the source file plus its hash in the job record.
...
- Follow the shared PS queue protocol for exclusive editing, saving and handoff. Within the same task, `WAITING_REVIEW` blocks starting the next asset: finish the actual current-hash technical verification and required whole/local visual self-check first. `PASS` permits continuation. After a complete real review returns `FAIL`, an exhausted budget or the user's explicit instruction to archive that candidate permits recording the failed asset as a retained candidate and continuing an independent asset. Its dependent successors and release remain blocked. A safe PS handoff may let other tasks use Photoshop while this task completes its review; it does not permit this task to skip review and batch-edit more assets.
...
3. If Photoshop cannot perform the required operation through MCP, or the corrected candidate still fails, return to the frozen source and allow a fresh generation only if the same scene/menu job has remaining budget (three total including the first).
...
This rescue changes the repair method, not the total budget. Keep the same immutable job ID across locations and stages. If no budget remains, retain the closest candidate and report its defect; do not call generation.
