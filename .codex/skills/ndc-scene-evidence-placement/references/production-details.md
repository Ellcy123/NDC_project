...
For a direct map pickup that disappears after collection, freeze the untouched `original_scene` first. Produce and approve the complete `carrier_without_prop` state before adding the pickup; this state contains the persistent carrier plus carrier-owned lighting/shadow and must remain valid after pickup. Then author the pickup as an independent RGBA layer containing only the pickup and every pickup-attributable contact/cast-shadow pixel. If the carrier already exists, record that `carrier_without_prop` is byte-identical to `original_scene` instead of regenerating it. This three-layer sequence does not apply to Type 7 secondary-menu children.

For a collectible scene pickup, start from a tight intent mask and expand it into the parent authoring workspace under the base skill's evidence rule: at least `3x` the proposed object bounds on both axes, at least `128 source pixels` on every unoccluded side, and preferably the whole usable tray, tabletop, drawer interior, or floor patch. After generation, derive a separate final composition mask from the actual object and its shadow; keep the support-surface repair only in the persistent `carrier_without_prop` state, never in the disappearing pickup layer. Keep at least `64 source pixels` around every unoccluded semantic edge. The composition mask must remain inside the parent workspace but must not include unrelated model drift merely because the parent workspace allowed it. Do not include characters or protected architecture. Never rescale or crop the full scene after placement.
...
For every in-scene pickup, clue prop, environmental-storytelling prop, freestanding carrier, or Type 6 container, run a separate `SHADOW_COVERAGE_GATE` before hotspot extrema are registered. The prop may remain after interaction; that never waives its shadow. Start from the accepted full native scene, not an inherited object crop; inspect the untinted parent at whole-image `100%` and local nearest-neighbor `200%` or greater. Record three independent masks: `BODY_MUST_COVER`, every visible attributable contact/cast-shadow region as `SHADOW_MUST_COVER`, and each foreground object as `FOREGROUND_MUST_EXCLUDE`. The final semantic target is `(BODY_MUST_COVER union SHADOW_MUST_COVER) minus FOREGROUND_MUST_EXCLUDE`. For a disappearing direct pickup, keep carrier-owned pixels/shadow outside this target and bind them to `carrier_without_prop`. A foreground object removes only the pixels it actually covers: if the target shadow remains visible on the far side, keep that continuation even when it becomes a disconnected Alpha island. The loose selection, four-extrema rectangle, base contour, and final post-exclusion Alpha must cover every visible must-cover region. Parent-pixel equality, non-empty Alpha, and internally consistent bounds cannot pass this visual gate when the visible shadow is clipped.
