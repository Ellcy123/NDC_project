...
输出Map、Type6、可点击子物品PNG、XYposition.txt、独立菜单XY、技术报告、逐热区审核和PS恢复件。所有入景道具均保留本体＋归属阴影覆盖结论；对地图直接拾取且拾取后消失的道具，过程包额外保留原场景、无道具承载物状态、独立道具＋阴影层、拾取前重建图、拾取后承载物保留／道具残留检查图及其哈希绑定。工程Sprite Physics Shape／PolygonCollider2D接入状态独立记录：图片和坐标通过不等于运行时热区或拾取消失行为已验证。完成后交给[最终复查](../ndc-prop-delivery-review/SKILL.md)。
