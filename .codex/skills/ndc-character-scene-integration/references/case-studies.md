...
## SC2206 layered test, 2026-09-08: rejected despite technical records

The user rejected the four-person hospital composite for elapsed production time, unreliable apparent support/scale, absent head-pillow response, inappropriate solemn acting and stiff bodies. Existing hash/geometry checks did not establish visual success.

The reusable failure was ordering and dependency handling: detailed local checks preceded a decisive whole-shot read; shared provisional scale assumptions appeared more conclusive than they were; complete-layer feet became invisible behind furniture; a responding pillow was treated as frozen background; quiet acting was reduced to similar closed postures. Generative extraction could redraw accepted people and trigger more registration work.

Apply the cadence, placement, layered-contact and directing references. Review the whole shot early, preserve same-layer support evidence, couple human/soft-object response, evaluate naturalism independently, and consolidate actual review evidence. The case remains rejected work history; this lesson does not make its earlier PASS records valid.
...
Correction: new production uses cast-scale v2 with `headScalePriority: true`. Measure each approved card's anatomical head/full-body ratio and compare individual/pairwise expectations after validated depth projection, then standing-equivalent body height. Recheck current final heads after generation using the user's 20% individual/pairwise tolerance. Alpha boxes, garments, props and regenerated supports do not determine scale. Investigate material contradictions with scene depth; small depth differences or approved identity proportions do not require strict raw screen-size ordering of every pair.

For later lying work, v2's use of `target.foot.y - horizonY` is historical only: a mattress contact is not a floor-depth sample. New or revised lying placements use cast-scale v3. Floor lying is declared explicitly; elevated lying binds the unchanged support contact to a scene/pose-specific projected-ground point or same-depth reference plus current whole/local visual evidence. This closes the false-oversize path without changing the horizon, support coordinates or tolerance.
