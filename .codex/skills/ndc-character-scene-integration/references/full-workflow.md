...
Use [execution-profiles-and-finalizer.md](execution-profiles-and-finalizer.md) as the controlling efficiency layer: one workflow, three execution profiles, seven universal checks, fact-resolved subchecks, early Layout/Pixel Proof previews, content-bound cache, per-phase timing and one finalization entry. The detailed steps below remain the source of the individual evidence contracts.
...
Run cast-scale v2 head-first against approved-card anatomical head/body references; use v3 for every new or revised scene containing a lying actor. A floor-lying actor explicitly declares ground support. An elevated lying actor keeps its authored bed/sofa/stretcher/table contact point, but derives perspective depth from a separate ground-plane projection or an exact same-depth actor. Bind that derivation to the fixed scene hash, current placement hash and pose ID, named support plane/object, independent plane/projection evidence, and current whole/local exact-pose visual review. Do not rewrite `target.foot`, horizon or tolerances to simulate floor depth. Existing accepted reuse remains historical evidence unless geometry changes. Use 20% individual and 20% pairwise head-ratio deviation under the user's 80% consistency requirement. Retain canonical height and fixed-scene body-scale checks. For a tilted/lying head, use the reviewed crown-to-chin anatomical axis; do not enlarge it to compensate for screen rotation or pretend the axis solves unseen foreshortening.
...
Second-stage formal pixels are generated only on ChatGPT web through a supported visible browser (`iab`, Chrome, or Edge), operated by the Terra/xhigh production task. This web path is required to preserve the established character style. Follow [the production web-generation contract](../../ndc-character-scene-production/references/chatgpt-web-generation.md): build a hash-bound submission packet, then upload the exact three references and full prompt in fixed order. Do not call Codex image generation or an image API as a fallback. Browser choice changes only submission and recovery; all prompt, reference, geometry, budget and acceptance rules below remain unchanged.
...
Default first output is contextual local replacement. Review it against identity/style authority and the approved pose/scene, choose model retry versus later bounded repair, and freeze every usable source with its receipt, review and hash. Prefer continuing through all required actors/states in the same scene before formal-role component extraction, Remove Background, Select Subject, Alpha/mask work, registration or compositing. If web generation or recovery is genuinely waiting／blocked, or another concrete scheduling reason is logged, already-ready actors may enter compliant extraction while missing actors remain tracked. Preserve context for contacts and lighting; only actors and authorized minimum soft responses may later become replacement pixels. Do not regenerate fixed furniture into an actor layer.

Maintain a scene-cast generation progress manifest against the frozen scope, listing every required actor/state, its generated or validly reused source, receipt, review, hash and current status. A complete manifest is the preferred transition point for applying the PS-first repair decision and extraction routes across the scene cast, but it is not a hard gate. When a logged scheduling exception applies, a ready candidate may enter this post-generation work while later characters remain ungenerated; the incomplete manifest stays open and cannot support whole-scene completion. Existing partial extraction from an older interrupted run remains valid and may be continued under the same scheduling rule.

### Extraction and registration

Use verified extraction that preserves accepted RGB; normally begin after the current scene/revision generation manifest is complete, or record the scheduling exception before processing an already-ready actor. Do not require a second generative pass when genuine alpha or a supported non-generative selection/mask route already works. Keep the original complete contextual canvas and its scene-crop mapping. A model removal fallback consumes the actor/interaction's shared model budget and must be inspected for redesign or scale drift.
...
Same-image technical steps can share one coherent operation ending in save, technical check and whole/local visual review before this task advances to its next image. A safe saved checkpoint and frozen review snapshots permit handoff to another independent task without accepting the unfinished image or its dependents, provided the native Photoshop MCP has no in-flight or unknown command. Retain actual extraction and registration artifacts/views for the legacy ledger scopes; do not fabricate a discarded stage. Preserve the high-resolution source and compose transforms before final resampling.
