...
For the unified `probe` / `provisional` / `formal` manifest and the `finalize-scene` command, read [execution-profiles-and-finalizer.md](execution-profiles-and-finalizer.md). This finalization entry resolves universal and factual checks, verifies hash-bound evidence, makes the two previews and batched matte board, runs whitelisted applicable validators, and optionally writes XY/reconstruction. It does not replace the detailed contracts below or infer artistic PASS.
