...
- Read/copy only from `D:/PMH/工作` and `D:/PMH/ndc`; outputs belong under `D:/Codex/NDC`. Keep evidence and provisional review packages under `工作过程文件/角色融入场景/Unit<n>` and passed assets under the corresponding `最终交付` category, inside the source scene basename.
