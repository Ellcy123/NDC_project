...
工具拒绝：非3:4母图、透明/无效输入、过期标注hash、非法点位、下巴不在眼下、上采样、越界裁切、危险文件名、覆盖已有输出、输出嵌套在Skill/已识别工程/只读源目录。工具不会改Alpha、修脸、自动判断表情或无白边；若需要Photoshop操作，仅走当前允许的原生MCP通道，并确认本机无其他PS操作者。同任务逐张保存并完成真实审核；PASS才推进相应依赖，FAIL在缺陷明确且原预算耗尽或用户指示封存后，保留证据并仅继续独立资产；可恢复保存及固定审阅快照后的跨任务挂起释放不改变本图状态或配套big/small依赖，离线裁切工具本身不占PS。
