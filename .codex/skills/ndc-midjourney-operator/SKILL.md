...
For a selected local asset, save an `ndc-texture-coherence/v1` record and validate it with `python scripts/art_pipeline/ndc_art.py tool texture`. The validator proves evidence completeness only; browser review without a downloaded local asset must still report the same named checks and remains incomplete for formal file delivery until its record is validated.
<!-- NDC_TEXTURE_COHERENCE_MODULE:END -->
