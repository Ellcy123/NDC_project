...
提取按三个事件推进，而不是等待一份“完美”文件后才做所有下游：

1. 第一份通过 H0 结构的可用 RGBA 到场，立即贴回实际场景并叠加当前真实 UI，形成 Pixel Proof Preview；H1/H2 的少量 halo、残色、发丝/半透明边缘或局部衣鞋边问题可标记 provisional，不延误该预览。
2. 第一份正式 RGBA 通过后，停止同一角色/状态的剩余提取路线，不为了填满日志继续操作。
3. 没有正式 RGBA 时，继续所有其它适用且实时 `supported` 的独立路线；只有全部已有实测、`NOT_APPLICABLE` 或 `UNSUPPORTED` 证据，才可登记整体能力失败。

Alpha 审核分为 H0 结构与 H1/H2 边缘质量。H0 要求真实透明、无矩形场景背景、无重大人体/关键服装或道具截断；H1/H2 处理小型边缘缺陷。`provisional` 与 `formal` 各用一张批量 contact sheet 同时展示黑底、白底、从实际场景取样压暗的深场景色底，并完成一次哈希绑定审阅；这三种底色不是三个独立手工阶段。实际场景关系由 Pixel Proof Preview 审阅。
...
只有 `EXTRACTION_CAPABILITY_GATE: PASS` 才能由本机唯一当前操作者开始提取。随后仍须实际导出 PSD 与 PNG、检查 Alpha 像素、边缘、尺寸、坐标和全场回放；预检通过不等于透明资产通过。
...
该流转只提供可审核资产，不是正式放行：源文件和完整证据不得写入已批准正式资产区，不得满足正式发布验证器，不得把失败项改记 PASS。若当前任务已明确选定其中一张图准备交付，可按共享 delivery-candidate registry 将其字节一致副本写入 `{DELIVERY_ROOT}` 的 `交付候选` 隔离层，状态仍为待复核／provisional，不能作为正式资产或工程同步源。用户审核、人工修正或后续自动提取通过后，从保留原图或明确选定版本建立新修订并重跑受影响门禁；旧 provisional 版本继续作为历史，不被覆盖。若根本没有合规 RGBA 输出，则不适用本分支，按下节能力失败处理。
