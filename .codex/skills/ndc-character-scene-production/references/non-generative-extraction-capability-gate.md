...
若当前会话缺少受支持源图入口或所选路径任一能力，导致没有任何合规 RGBA 输出，写入 `EXTRACTION_CAPABILITY_GATE: FAIL` 与实际缺项，保留原始上下文候选及 SHA-256。暂停依赖该角色透明层的合成与最终放行，但继续同一整场内不依赖该透明层的已授权工作，例如其它冻结角色的网页生产、来源回执、候选审核和独立准备；涉及该角色精确遮挡、接触或最终合成的步骤继续等待能力恢复。已有可打开但不完整／低置信度 RGBA 时改走上一节，不得误报为“没有输出”。不得：
