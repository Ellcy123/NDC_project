...
网页限流、上传故障、会话失效或生图能力不可用时，同一原样操作最多重试一次；再次失败须缩小诊断、切换受支持浏览器或等待恢复。内置 `iab` 首次明确无法收图属于立即切换外置浏览器的回收故障，不消耗第二轮重复等待；在同一账户、同一对话完成验收和下载。提交前切换浏览器不计模型 attempt；提交已发生或结果未知时，只在同一专用对话核实。不得切回 Codex 图片生成或图片 API。保存当前 URL、browser、submission ID、pending/unknown 状态和已下载候选，继续不依赖该结果的场景记录、离线审阅或其它独立工作。确认原对话没有结果后，才按 journal 规则进入下一次尝试。
