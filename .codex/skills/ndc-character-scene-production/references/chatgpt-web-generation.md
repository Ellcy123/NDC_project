...
- 提交前浏览器不可用时，可切换到另一受支持浏览器并打开同一专用对话 URL；这不是模型 attempt。网页已经收到消息或状态未知时，换浏览器只能用于核实同一对话，不能重发。用户已授权实际人物入景生产时，冻结 submission packet 内的三引用上传及对应提示词提交属于已交办流程，不再逐角色、逐次或逐文件向用户请求“发送”确认；该授权不包含包外文件、其它场景或其它外部操作。
