...
一个场景 revision 以冻结 scope 中全部 `case / snapshot / actor_pose_id` 的并集为正式生成范围。Terra 默认按已确认的远近层次和批次顺序连续生成、回收并逐个完成提取前视觉审核；单个角色／批次完成后通常保存原始下载、网页回执、候选结论和冻结 SHA-256，并继续本场剩余角色。经当前来源规则允许直接复用的正式角色／状态，可用相同字节及复用证据满足对应范围，不为形式完整重复生成。

建立整场生成进度清单，逐项列出冻结 scope、每个必需角色／状态、生成或复用来源、原始文件、来源回执、提取前视觉审核、SHA-256 和当前状态。完整清单是统一切换到提取阶段的默认最优时机，不是硬门禁。若网页生成、回收或浏览器传输真实等待／受阻，或存在其它有证据的具体调度理由，可对已经就绪的角色先执行合规 Photoshop 提取和不依赖缺项的后续工作，以免空转。例外须在清单中记录原因、开始时间、已就绪／未就绪项、执行范围及恢复顺序；未就绪项继续生成或保持待办，不得从完整 scope 删除。

旧任务若已经对部分角色完成提取，不删除、不覆盖既有结果，并按当前调度继续。无论是否错峰，局部结果都不得被登记成整场完成；完整 scope、尚缺角色／状态、来源门禁、候选审核、提取能力、路线耗尽和最终验收门禁保持不变。该优先顺序只涉及正式角色生成后的抠图去背景链，不限制参考阶段为白模或生图输入执行已授权的准备工作。

## 多路线提取耗尽门禁

透明提取不能把一个按钮、一个动作或一个不合格输出当作 Photoshop 整体能力结论。每个角色开始提取时，无论是整场生成完成后的统一阶段还是有记录的错峰调度，都必须按当前《PS MCP 操作参考手册》第 4.3 节建立 `route_exhaustion_log`，结合当前命令目录与人物的硬边、发丝、半透明材质，登记原生 Alpha、直接透明动作、语义选区到 RGBA、通道／计算及其它受支持且适用的非生成路线。对每条端到端命令均为 `supported` 的适用自动路线至少在新副本上实测一次，并记录命令、回执、输出或精确失败点；同一原样命令最多重试一次，但必须换测其它独立路线。只有正式 RGBA 已通过，或所有适用路线均已有实测／不适用／不受支持证据，才可停止提取探索。
...
- 每次正式提交、结果回收和未知状态处理均按[网页版 ChatGPT 生图](references/chatgpt-web-generation.md)执行。一个完整场景 revision 使用一个专用 ChatGPT 网页对话；每次提交前先生成不可覆盖的 submission packet，按 `local-whitebox-crop → untouched-full-scene → approved-character-card → full-prompt` 固定顺序逐项核对并提交，不依赖历史上下文暗示。已授权正式生产中，包与门禁通过后由任务自动选择最高可用网页档位并直接上传、提交，不向用户逐角色或逐次询问“是否发送／请回复发送”。浏览器按用户指定及当前健康、登录和可追溯状态动态选择，不设置“先内置、再外置”的固定顺序；不同已登录 ChatGPT 账户均可继续任务。接管已有外置标签页时先枚举并按完整对话 URL 认领，动作报 session ownership 错误时重绑而不是判定浏览器失联。若当前账户可访问原对话／结果，仍只接续和核实原记录；若账户隔离导致原对话、历史消息或结果确实不可访问，则记录 `account_record_unavailable`，保留原尝试计数，在新账户建立新专用对话并以同一输入／提示词哈希重新生图。控制传输失效按引用中的有限重试和重置规则处理；完整缓存只能保存为非正式恢复候选。下载网页提供的原始结果到同一过程包，保存 v2 可追溯回执；浏览器预览截图、已加载缓存或恢复候选均不能冒充原始下载文件。
