...
- contextual结果先按重要度配置分流：H0 失败立即拒绝，H1 超过 10% 才修复或重试，H2/H3 在 20%/30% 上限内直接接受且不追加润色；禁止跨区域平均。随后进入 RGBA 提取前必须执行[非生成提取能力门禁](references/non-generative-extraction-capability-gate.md)，并保存 `EXTRACTION_CAPABILITY_GATE: PASS` 回执。门禁以可复核的**结果**为准，不指定“选择主体”或任何单一 Photoshop 按钮。可使用真实主体/对象/快速选择与图层蒙版、已安装的非生成去背景动作、色彩范围、通道/计算得到的 Alpha、既有蒙版修整、Select and Mask，或已明确指定人工制作人员完成的 Photoshop 精修；每种路线均须保存对应的原生操作、PSD/PNG 和 Alpha/蒙版证据。`selection.polygon`、自动坐标轮廓、粗略矩形、像素复制、仅切换图层可见性，以及动作成功/棋盘格/透明像素统计本身，均不得作为人物边缘识别或透明提取完成的依据。动作可在源图未保存副本上成为正式候选路线，但只有其实际输出无场景残片、无生成像素，并通过全身及发丝/半透明边缘、手、鞋和高对比衣物边缘的局部审阅后才可放行；不要求再把动作结果强制改造成“主体选择”。人工 `Select and Mask`、通道或笔刷修整必须满足上文的人工制作人员授权边界，并登记交接及保存的 PSD/PNG；不能以“之后人工处理”替代结果证据，更不能默认要求用户执行。一个自动提取候选不合格时，先按实时能力在其它合规非生成路线中继续，不能从一次失败直接升级为用户任务。所有路线均不可用时，以“提取能力/边缘质量”失败登记，不计为正式角色生产。该角色依赖透明层的合成和最终放行必须暂停，但应保存检查点并继续同一整场内不依赖该透明层的已授权工作，例如其它已冻结角色的网页生产、来源回执、候选审核和独立准备；不得省略受阻角色、伪造 RGBA，或把未完成整场登记为 PASS。任何路线均须导出 PNG RGBA 并随后实测 Alpha；仅“Photoshop 已连接”、仅能打开/导出文档或尚未验证的人工意图均为 `FAIL`。通过后才执行非生成抠图、图层顺序和已有像素修复；所有PS连续段使用本机已配对原生 MCP，确认无在途/未知命令并安全保存审阅快照后才交棒。
