...
Only in `USER_AUTHORIZED_PHOTOSHOP_MCP`, when Photoshop MCP actually exports an openable non-generative RGBA but extraction is incomplete or low-confidence, preserve the exact pre-extraction original and SHA-256, attempted RGBA, recoverable PSD, mask/path/action evidence, multi-background previews, confidence and defect locations. Mark it `PROVISIONAL_EXTRACTION_PENDING_USER_REVIEW`; do not leave the expression blank. Continue provisional E7–E9 profile composition, guide/set review and packaging from that exact attempted foreground, with every derivative inheriting the status and source hash. Deliver the original, attempt, evidence and derivatives in a dedicated user-review package under `工作过程文件/角色表情/Unit<n>`; do not enter E10 formal release, `最终交付`, or schema-13 PASS. Never overwrite the original, an older provisional version, or an approved sibling. After user review or a passing new extraction, create a new revision and rerun affected Alpha, profile, set and receipt gates. This exception does not alter `USER_MANUAL`, does not authorize PS by itself, and does not waive artistic or source-anatomy failures. If no compliant RGBA was produced, keep the ordinary capability block.
...
The sole downstream exception is the explicitly authorized Photoshop MCP provisional branch above: it may continue E7–E9 only after the failed/uncertain current-hash review and defects are truthfully saved, and every output remains non-final. This is review-package progression, not stage acceptance or a visual PASS; formal validators must continue to block it.
...
- In authorized PS mode, an actually exported but incomplete or low-confidence RGBA is preserved and carried through the marked provisional review-package branch instead of being left blank; it remains ineligible for formal release until corrected and rechecked.
...
7. `E6_USER_RETURNED_RGBA_INGEST`: in `USER_MANUAL`, after the user confirms that those exact handoff files were edited in place, verify their pre-edit manifest mapping and unchanged subject content, build multi-background previews, and pass `ALPHA_EDGE_GATE` without modifying Alpha. In `USER_AUTHORIZED_PHOTOSHOP_MCP`, ingest the actual PS output; a failing or uncertain but openable RGBA may branch to provisional E7–E9 under the rules above, never to E10.
...
An authorized PS provisional review package is delivered separately under `工作过程文件`; it must include the exact pre-extraction original and all inherited provisional derivatives, and it cannot satisfy any item in the formal checklist above.
