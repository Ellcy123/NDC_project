...
Use `identity-and-expression-calibration.md` to distinguish already-present calm cues from the planned delta and to separate fixed face geometry from movable performance.
...
Check facial geometry and viewpoint before expression acceptance; then compare the candidate with calm and accepted siblings at the job's thumbnail target. A user rejection invalidates that asset's earlier PASS and downstream use; apply the versioned supersession procedure in `identity-and-expression-calibration.md`.
...
Map every in-place edited file to its exact pre-edit row in the explicitly current handoff-manifest revision. Verify that the user performed the background processing, the canvas was not rescaled or cropped, and the protected subject content remains intact. A processed RGB candidate will have a changed hash; an already-RGBA approved calm may legitimately remain byte-identical after user confirmation. The immutable manifest row preserves provenance; a changed hash alone proves neither user completion nor valid Alpha. Codex must not use Photoshop, `remove_expression_background.py`, another extraction script, or an Image model to repair Alpha or white fringe.
...
Create schema-12 manual profile receipts or schema-13 receipts for explicit PS processing (see `receipt-schema.md`). Stage the complete package under `工作过程文件`, enumerate every required `character + expression + profile`, and run `audit_expression_delivery_release.py`. Copy to `最终交付` only after `RELEASE_STATUS: PASS`.
