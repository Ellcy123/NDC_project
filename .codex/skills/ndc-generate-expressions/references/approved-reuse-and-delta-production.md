...
At E0, do not limit discovery to the current Unit or to already-known role folders. For every Unit3-or-later expression request, first run the device preflight, then inventory the user-designated read-only roots in this fixed order: `expression_sources.unit1_expression_root`, then `expression_sources.unit2_expression_root`. Do this before creating a generation queue or expanding discovery to another user-authorized archive or project index. If either required designation is absent or unavailable, report `MISSING_LOCAL_EXPRESSION_SOURCES`, identify the missing role and ask the user to specify its directory; do not silently change priority. If neither designated library supplies a matching candidate, continue with any other user-authorized historical root as needed. Discovery must match the same character through reviewed role aliases, then match exact expression IDs or explicitly declared semantic aliases, and classify the profile from the path. A same-named expression belonging to another character is reference-only and can never be reused; a merely similar emotion is not a reusable match.
