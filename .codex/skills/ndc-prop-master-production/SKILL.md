...
- **道具候选的返工路由：** 先核对道具本体的身份、结构、必要信息、指定状态与可读文字。角色/手部、背景、承托物、场景残留、外部构图边界、透明背景和其他非道具本体问题，按当前手册第 4.3 节建立 `route_exhaustion_log`，对全部材质适用、权限内且端到端命令为 `supported` 的 Photoshop MCP 非生成抠取／移除路线逐条实测并视觉检查；当前命令确实不支持所需操作时，记录该路线的能力边界，并在本 Skill 已授权时尝试一次不改写道具像素的确定性 Alpha 路线。同一原样命令最多重试一次，但单个动作、按钮或路线失败不得作为整体停止理由。任何实际隔离结果不完整、视觉失败或置信度不足时，保留提取前原图和尝试件，标记 `PROVISIONAL_EXTRACTION_PENDING_USER_REVIEW`，按下文继续 provisional 下游与待用户审核交付，同时继续尚未实测的适用路线；它不计正式 PASS 覆盖，不可正式发布。正式 RGBA 通过后可停止其余路线，否则只有路线耗尽日志完整时才能登记能力失败。随后可在剩余额度内生成空背景/原生透明背景的独立道具图，但不得围绕同一失败方法反复尝试 Alpha 阈值或描边。道具本体语义错误同样直接回到生成且不适用 provisional 提取例外；确定性蒙版、裁切和变换只能提取或整理既有像素，不能改写道具本体、必要信息或文字。
