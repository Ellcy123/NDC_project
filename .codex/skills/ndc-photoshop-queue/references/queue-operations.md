...
原生 `photoshop_job_*`、用户辅助作业和显示式编辑对话框不属于自动队列生产路径；这些接口不能通过换名字绕过限制。Bridge 初次安装、设备级密钥迁移或已证实密钥 ACL 损坏时，用户明确要求恢复连接后，可在队列完全空闲且 Bridge 尚未配对时调用 `photoshop_pairing_begin`，由受信任的本机对话框显示短时配对码；已有配对、owner、waiting、external 或原生 handler 时一律拒绝。配对只恢复认证，不授予图像操作租约。所有 Photoshop 内操作仍走已批准 MCP 通道，不调用原始宿主执行后门。
