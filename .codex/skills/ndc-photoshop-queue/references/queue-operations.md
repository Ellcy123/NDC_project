...
把 `$psTaskId` 换为当前任务／会话的真实 ID，不能使用通用示例 ID。Node 必须支持 `node:sqlite`；公共入口优先使用 `NDC_NODE_EXE`，其次使用 PATH 和当前用户的 Codex 内置 Node。不得把 Windows Store `python.exe` 的无输出或退出码 9009 记成 PS MCP 故障；需要 `ndc_art.py` 时必须先解析到真实 Python，或直接使用上述不依赖 Python 的队列入口。`scripts/runtime-binding.json` 只记录可提交的相对约定与受审哈希；PS MCP 外部运行时默认从 `%LOCALAPPDATA%` 解析，非标准安装设置 `NDC_PS_MCP_RUNTIME`。允许目录优先来自显式 `NDC_PS_ALLOWED_ROOTS`；未设置时，`runtime-config.mjs` 自动读取随工程本机维护且不提交的 `ndc.local.json` 中 `work_root`、`engine_root`、`planning_root`，单项环境变量仍可覆盖对应本机字段。队列状态默认在当前用户 `%LOCALAPPDATA%/NDC/photoshop-queue`。
...
原生 `photoshop_job_*`、用户辅助作业和显示式编辑对话框不属于自动队列生产路径；这些接口不能通过换名字绕过限制。Bridge 初次安装、设备级密钥迁移或已证实密钥 ACL 损坏时，用户明确要求恢复连接后，可在队列完全空闲且 Bridge 尚未配对时调用 `photoshop_pairing_begin`，由受信任的本机对话框显示短时配对码；受限桌面无法显示该对话框时，同一入口可把短时本机 `端口-配对码` 直接写入 Windows 剪贴板，但绝不在 MCP 返回、日志或聊天中披露。已有配对、owner、waiting、external 或原生 handler 时一律拒绝。配对只恢复认证，不授予图像操作租约。所有 Photoshop 内操作仍走已批准 MCP 通道，不调用原始宿主执行后门。
