...
人物入景以确定并交接整场白模为上游完成边界；下游 Terra/xhigh 任务全权负责正式角色与白模对齐、通过受支持的内置 `iab` 或外置 Chrome/Edge 在网页版 ChatGPT 生图、结果回收、生产自检、返修和交付，禁止调用 Codex 图片生成工具、图片 API、其它站点或其它生图后端。同一场景必须先完成并冻结全部必需角色／状态的正式生成或合法复用源，再统一进入抠图、去背景、Alpha、注册和合成；不得在角色间交错“生成一个、提取一个”。上游交接后继续其他场景，不持续监控或代做生产验收；全部交接后结束参考轮次。下游自行登记结果并接续 READY 场景，仅白模本身的核心错误主动回传。详细责任见[入景专属协议](references/integration-pipeline.md)，此边界不改变 UI/MJ 的阶段分工。
...
每个流水批次默认一个上游协调任务、一个下游阶段目标任务。同场景多视角共同归这个下游；人物入景也按完整场景交接，使用 [入景专属协议](references/integration-pipeline.md)：Astra/medium产出所有参考，Terra/xhigh统一编排所有同场角色并在受支持的内置或外置浏览器中通过网页版 ChatGPT 生成正式像素；不能按角色拆分，也不能在同场景内让已生成角色越过尚未完成的角色提前进入提取阶段。未到交接条件的单位保留在完整总清单中，不能删除缺项来提高完成率。
