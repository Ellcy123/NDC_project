...
def worker_prompt(reservation, controller_task_id, core_script):
    ...
    if mode == "validation":
        effect = "本次仅验证派发、领取、版本与回传协议；只允许在独占工作目录处理明确标为合成夹具的文件。不调用 Midjourney、网页版 ChatGPT 生图、Codex ImageGen 或 Photoshop，不制作真实生产资产，不写正式交付，不填写虚假的视觉 PASS。"
        ...
        effect = "使用 gpt-5.6-terra / xhigh 执行 ndc-character-scene-production。接收一个完整场景的全部状态和人物参考，沿用原生产ID、原次数与真实前后置门禁；整场由你这一任务统筹，不拆成多个互不知情的 Codex 角色任务。正式像素只通过受支持的可见浏览器（内置 iab，或用户已指定/授权的外置 Chrome、Edge）在网页版 ChatGPT 生成；用户指定浏览器优先，未指定时选当前健康、已登录且可追溯的浏览器，不同已登录账户均可继续。每个 scene revision 建立一个逻辑场景工作区，不强制独占物理 OS 窗口；工作区内为不同角色／状态生成单元建立多个专用 ChatGPT 对话，一次真实 canary 通过后按 WIP 上限逐标签提交，不等待前一角色出图，使不同对话同时生成。每个对话仍只允许一个未决提交，全部对话分别重传三引用、完整提示词和 production Skill 内用户原版风格 asset 的精确字节；禁止旧 style-lock、转录或改写版。下载原图并保存 logical workspace、generation unit、browser、账户非敏感标签与 v4 网页生图回执。原记录可访问时只回对应单元原对话核实；账户隔离导致旧记录确实不可访问时，以 account_record_unavailable 保留旧 attempt，在同一逻辑场景工作区登记后继对话并以相同输入／提示词／style-lock 哈希登记新的真实 attempt。禁止调用 Codex ImageGen、图片 API、其它站点或其它生图后端。姿势、几何、多人关系的参考错误回传 reference 阶段，由 Astra / medium 修正；技术提取、边缘、映射问题在原预算内处理。实际 PS 只使用本机已配对原生 MCP，并保持本机单一操作者；不调用旧队列/租约，不抢写上游参考或其他场景。"
        effect += (
            "并发范围包括不同角色，以及同一角色的不同 pose；同一 actor/pose 未决时禁止重复提交，终态后的预算内返修沿用原专用对话。"
        )
    else:
...
def build_dispatch_plan(reservation, project_context, *, core_script=None, python=None):
    ...
    plan["effect_scope"] = {
        ...
        "character_scene_generation_backend": "chatgpt_web_browser" if integration and mode == "production" and not reasons else None,
        "codex_image_generation": False if integration else None,
        ...
        "photoshop_backend": "native_mcp_single_operator" if integration else None,
        "photoshop_requires_shared_queue": False,
