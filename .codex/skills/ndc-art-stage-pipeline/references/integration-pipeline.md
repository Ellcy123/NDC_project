...
上游把整个场景及所有必需角色/状态的白模确定、冻结并成功交接后，完成该场景参考职责，立即推进下一独立场景；全部参考交接后结束本轮。下游全权负责正式角色生成、与白模的位置及比例对齐、头身比/动作核验、完整母层、遮挡、提取合成、自检、返修和交付。上游不持续监控、轮询等待、逐角色审查生产结果或代做生产修复。结果由下游自行写入原流水并接续下一 READY 场景；结果登记是下游职责，不要求上游常驻回收。
