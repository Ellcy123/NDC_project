...
上游把整个场景及所有必需角色/状态的白模确定、冻结并成功交接后，完成该场景参考职责，立即推进下一独立场景；全部参考交接后结束本轮。下游全权负责正式角色生成、与白模的位置及比例对齐、头身比/动作核验、完整母层、遮挡、提取合成、自检、返修和交付。下游内部顺序固定为：先生成、回收、审核并冻结同一场景全部必需角色／状态，再统一进行提取、去背景、Alpha、注册与合成；禁止角色 A 生成后先抠图、角色 B 随后才生成。上游不持续监控、轮询等待、逐角色审查生产结果或代做生产修复。结果由下游自行写入原流水并接续下一 READY 场景；结果登记是下游职责，不要求上游常驻回收。
...
正式生图按远近批次执行，但批次只控制提交顺序。每个网页结果回收后只完成提取前视觉审核、保存原始下载与来源回执并冻结 SHA-256，然后继续本场剩余 `actor_pose_id`；不得触发正式角色的 Remove Background、Select Subject、Alpha／蒙版、注册、接触或最终合成。冻结 scope 的全部必需角色／状态均已有审核通过的生成结果，或已有当前规则允许的正式复用源后，建立 `SCENE_CAST_GENERATION_GATE: PASS`。门禁逐项绑定 scene/revision、完整 scope、actor/pose、生成或复用来源、原始文件、网页回执、视觉结论和 SHA-256；缺失、失败、待回收、结果未知或未冻结任一项均为 FAIL，并阻断该场景全部新的提取操作。不同完整场景可在隔离条件下独立推进。

旧任务已有部分提取结果时保留其文件和历史，不回滚或覆盖；若本场仍有角色／状态未完成生成，从恢复点起停止新的提取，先补齐整场生成冻结门禁。该门禁不阻止参考白模与正式生图输入所需的已授权准备工作。
