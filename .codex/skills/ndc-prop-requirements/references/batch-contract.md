...
1. 来源和权限优先：项目来源检索顺序、只读输入、PS MCP通道和逐图审核保留。PS占用、保存交接及检查调度遵循共享PS协议：同任务WAITING_REVIEW时不能开始下一资产；安全交接后其他任务可使用PS。完整真实审核FAIL后，预算耗尽或用户明确封存候选时，可记录封存并继续独立资产；失败资产的后继依赖及发布仍阻塞。过程与恢复文件放项目工作过程目录；正式 PASS 与共用执行核心定义的隔离“交付候选”可进入最终交付，但二者必须分树、分状态、分统计。未授权不改Unity。
...
4. 第二阶段到第三阶段按场景及完整关联前置放行：某场景的全部道具、同物状态、容器内容及物理关联所需母版／普通Big、真实外部父图均有效通过，照片内容和跨物品引用事实及来源已锁定，且没有未解决关联，即可制作该场景。只引用已确认身份／日期等事实时，不附加生产该引用对象其余母版的要求。无关场景的未完成图不阻塞它。第三阶段全部非Icon必需图完成后仍统一制作Icon；但某场景自身的第三阶段非Icon产物、场景预览与菜单定稿冻结后，可先进入该场景第四阶段热区，不等待无关场景或全批Icon。Icon包括其专用高分辨率来源，不在第二阶段抢先生产。环境痕迹不强行生成独立物件。具体索引和迁移见[场景放行合同](scene-release-contract.md)；无索引的旧批次保持全批次门槛。
...
- 按场景放行不等于逐道具抢跑：尚不可执行的缺失项记录本场景全部关联中缺哪个前置；不能仅按scene_id过滤母版，也不能用完成百分比放行。达到当前场景门槛只复检和修复必要前置；其他独立场景可以继续。Icon仍按全批第三阶段生产；第四阶段入口可按冻结场景释放；最终交付仍用整批门槛。缺失项本轮产生的失败应在原job额度内闭环，不能把它误当作禁止处理的旧资产返修。
...
状态写入不得依靠手工编辑大体积 `batch.json`。使用交付复核 Skill 的 `scripts/batch_update.py`，以 `--expect-batch-sha256` 和 `--expect-status` 做并发保护，绑定当前输出／审核哈希并追加哈希串联的 `state_events`。`next_action` 只保存一至三条当前可执行动作、合计不超过 300 字；历史说明放状态事件或过程记录，不继续堆入活动队列。完整报告由只读命令从 batch、候选清单和审核证据重算。
...
进度与审计的读取顺序固定为：先运行 `delivery_candidate.py audit` 枚举明确交付候选及选定参考，再用冻结 active scope 判定缺口，最后按需读取对应场景工作包和过程证据。交付候选覆盖、候选复核通过、复核失败、有效 PASS 和正式交付就绪分列；同一必需产物只能有一个 active candidate。候选复核失败前不得移出，失败后仍须显式执行保留式迁移并记录去向与原哈希，不自动删除。
...
stage 2校验需求已锁；有场景索引时stage 3加--scene核验完整关联前置，attempt按job和产物归属自动执行同一门槛；stage 3不加--scene仍保留全局核验。stage 4加--scene检查本场第三阶段非Icon结果与场景／菜单冻结状态，不加--scene仍检查整批结果；stage 5始终检查所有必需结果及其依赖。不得缩小batch.scope/jobs来获得场景放行，不把原场景中尚待制作的菜单误列为入景前置。仅修复单一热区的任务可以在第一阶段明确限定为单资产批次，并纳入其既有场景父图；不能把一个已锁定全量任务偷换为该范围。
