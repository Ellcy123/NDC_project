...
def main() -> None:
    ...
    for key, box in zip(("front", "left", "back"), fullbody_boxes):
        paste_contained(
            canvas,
    ...
    paste_contained(
        ...
        scaled_box((2320, 720, 1400, 520), sx, sy),
    )

    if details:
        gap = 36
        area_x, area_y, area_w, area_h = 2300, 1300, 1420, 760
    ...
    if args.qa_overlay_out:
        ...
        left_x = round(30 * sx)
        right_x = round(2220 * sx)
        fullbody_y = round(45 * sy)
        fullbody_height = round(2070 * sy)
