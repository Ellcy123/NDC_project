...
- 用户指定 `iab`、Chrome 或 Edge 时优先使用该浏览器；未指定时按当前健康、登录和可追溯状态动态选择，浏览器载体没有固定先后顺序，不同已登录 ChatGPT 账户均可继续任务。提交前当前载体不可用时，可切换受支持浏览器或账户；未发生提交时不计模型 attempt。原专用对话／结果可访问时只接续该 URL、验收和下载，不新建对话、不重新提交；新账户因账户隔离确实没有原对话、历史消息或结果时，才按“账户切换与记录缺失”封存旧状态并在新账户新建专用对话重新生成。登录／验证码仍按人工节点处理，不输入、搬运或保存凭据。用户已授权实际角色生产时，冻结提交包内引用上传及对应提示词提交属于已交办流程，不再逐角色、逐次或逐文件向用户请求“发送”确认；该授权不包含包外文件、其它角色范围或其它外部操作。
...
- 记录不含凭据的 `account_profile_label`、browser、切换原因和时间；不得记录邮箱全称、密码、验证码、Cookie、令牌或其它认证数据。账户不同本身不构成暂停理由。
- 原专用对话／结果可访问时，继续沿用原 URL、submission ID、attempt 与回执，不得因换账户重复提交。
- 只有新账户确因账户隔离看不到原专用对话、历史消息或结果，且已保存最后可知状态和核实证据时，才把原 pending/unknown resolve 为 `account_record_unavailable`；该终态保留原尝试并继续占用一次真实 model attempt，不记 `no_output` 或工具错误。
- 新账户重新生图时，以原冻结引用和完整提示词的相同哈希建立不可覆盖的后继 packet，并记录旧 manifest 路径／SHA-256、旧 submission ID、旧 URL、缺失证据、新账户非敏感标签和新 URL。新提交按新的真实 model attempt 计数并受原 journal 剩余额度约束；旧记录后来恢复时只做审计或结果回收，不覆盖、不补发、不合并成同一次尝试。

网页限流、登录／验证码、上传故障或结果未知时，只阻断依赖该提交的分支。原样操作最多重试一次；再次失败后可在提交前切换受支持浏览器／已登录账户，或保存 browser、账户非敏感标签、URL、submission ID、pending/unknown 状态和已有候选，继续不依赖该结果的工作。原记录可访问时先回原对话核实，不得重发；只有账户隔离导致原记录确实不可访问时，才按上一节以 `account_record_unavailable` 封存并在新账户发起新的计数尝试。禁止静默切回 Codex 生图／图片 API。
