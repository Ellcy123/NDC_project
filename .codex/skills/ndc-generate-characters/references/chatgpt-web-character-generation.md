...
- 用户指定 `iab`、Chrome 或 Edge 时优先使用该浏览器；未指定时选择当前健康、已登录且可追溯的受支持浏览器。提交前可切换浏览器并打开同一专用对话 URL，不计模型 attempt。内置 `iab` 首次明确确认无法回收已生成结果时，不继续重复等待：立即转到已登录**同一 ChatGPT 账户**的受支持外置 Chrome 或 Edge，打开同一专用对话 URL 验收并执行网页原始下载。该切换只接续原对话，不新建对话、不重新提交，也不新增模型 attempt；外置浏览器未登录同一账户或触发登录／验证码时按人工节点处理，不输入或搬运凭据。网页已收到消息或状态未知时，换浏览器只用于核实同一对话，不能重发。用户已授权实际角色生产时，冻结提交包内引用上传及对应提示词提交属于已交办流程，不再逐角色、逐次或逐文件向用户请求“发送”确认；该授权不包含包外文件、其它角色范围或其它外部操作。
...
网页限流、登录／验证码、上传故障或结果未知时，只阻断依赖该提交的分支。原样操作最多重试一次；再次失败后可在提交前切换受支持浏览器，或保存 browser、URL、submission ID、pending/unknown 状态和已有候选，继续不依赖该结果的工作。内置 `iab` 首次明确无法收图属于立即切换外置浏览器的回收故障，不消耗第二轮重复等待；在同一账户、同一对话完成验收和下载。恢复时先回到原对话核实，确认没有结果才进入下一尝试；禁止刷新后盲目重发、另开对话或静默切回 Codex 生图／图片 API。
