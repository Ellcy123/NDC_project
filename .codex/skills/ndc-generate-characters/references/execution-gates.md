...
先按 `character-contract.md`、`production-budgets-and-dependencies.md` 和 `production-record.md` 处理复用、实际依赖、证据继承与有限累计预算。下文只读取当前执行阶段的专门门禁，不因历史完整顺序加载无关分支；原样复制不重做未变化的艺术判断。
...
严格按真实依赖关系推进，每次处理明确的可验收对象；失败只阻断真实依赖的后代。同任务PS当前图完整保存、完成真实自检并核对hash；未审完不推进下一张，PASS才推进相应依赖，FAIL按全局队列有据封存后仅继续独立资产；跨任务安全挂起释放遵守 [全局队列](../../ndc-photoshop-queue/SKILL.md)，未通过资产及依赖仍阻断。正式交付链必须逐阶段通过；用户明确要求完整流程、探索或候选集时，可在失败项留痕后继续生成下游候选，但候选链不构成正式资产。
...
- 用户未明确要求 `4K` 或独立模块生成／拼版时，角色卡默认使用 `prompt-library.md` 的锁定整卡提示词一次生成；不得自动拆分模块。
- 用户明确要求 `4K` 或独立模块生成／拼版才进入模块化，记录触发原文及目标尺寸；单独模块请求不强制4K，视角与DAG见 `production-budgets-and-dependencies.md`。
- 肖像与默认角色卡生成提示词必须逐字匹配源代码块。技术规格不得附加到生成提示词。
- 每个模块独立生成与验收，按真实DAG排队；基础源失败阻断其后代，侧视失败不阻断无依赖的背面／头部或细节。全部要求模块通过后才拼版；同任务PS单图验收门禁保留，跨任务资源释放按共享队列执行。
