...
严格按真实依赖关系推进，每次处理明确的可验收对象；失败只阻断真实依赖的后代。同任务PS当前图完整保存、完成真实自检并核对hash；未审完不推进下一张，PASS才推进相应依赖，FAIL在缺陷明确且原预算耗尽或用户指示封存后，保留证据并仅继续独立资产；跨任务安全交棒须保存可恢复文件并确认无在途或未知命令，未通过资产及依赖仍阻断。正式交付链必须逐阶段通过；用户明确要求完整流程、探索或候选集时，可在失败项留痕后继续生成下游候选，但候选链不构成正式资产。
