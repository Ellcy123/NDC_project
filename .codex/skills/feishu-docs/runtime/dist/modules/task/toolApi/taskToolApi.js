...
import { TaskCreateBatchSchema, TaskDeleteBatchSchema, } from '../../../types/taskSchema.js';
function toNested(item) {
    const members = [];
    if (item.assigneeIds?.length) {
        members.push(...item.assigneeIds.map((id) => ({ id, type: 'user', role: 'assignee' })));
    }
    if (item.followerIds?.length) {
        members.push(...item.followerIds.map((id) => ({ id, type: 'user', role: 'follower' })));
    }
    return {
 ...
 * 批量创建飞书任务（含嵌套子任务）
 */
export async function createTasks(taskItems, api) {
 ...
 * 列取飞书任务（我负责的）
 */
export async function listTasks(options, api) {
 ...
 * 更新飞书任务
 */
export async function updateTask(params, api) {
    const { taskGuid, ...rest } = params;
    const hasFieldUpdate = rest.summary !== undefined ||
    ...
    if (!hasFieldUpdate && !hasAdd && !hasRemove && !hasAddReminder && !hasRemoveReminder) {
        throw new Error('至少需要一项更新：summary/description/due/completed_at/repeat_rule/start/mode/is_milestone、成员 add/remove、或提醒 addReminderRelativeMinutes/removeReminderIds');
    }
    ...
    if (hasFieldUpdate) {
        ...
        if (rest.summary !== undefined)
            updateParams.summary = rest.summary;
        if (rest.description !== undefined)
            updateParams.description = rest.description;
        if (rest.dueTimestamp !== undefined)
            updateParams.due = { timestamp: rest.dueTimestamp, is_all_day: rest.isDueAllDay };
        if (rest.completedAt !== undefined)
            updateParams.completed_at = rest.completedAt;
        if (rest.repeatRule !== undefined)
            updateParams.repeat_rule = rest.repeatRule;
        if (rest.startTimestamp !== undefined)
            updateParams.start = { timestamp: rest.startTimestamp, is_all_day: rest.isStartAllDay };
        if (rest.mode !== undefined)
            updateParams.mode = rest.mode;
        if (rest.isMilestone !== undefined)
            updateParams.is_milestone = rest.isMilestone;
        result = await api.updateTask(taskGuid, updateParams);
    }
    if (hasAdd) {
        const members = [];
        if (rest.addAssigneeIds?.length) {
            members.push(...rest.addAssigneeIds.map((id) => ({ id, type: 'user', role: 'assignee' })));
        }
        if (rest.addFollowerIds?.length) {
            members.push(...rest.addFollowerIds.map((id) => ({ id, type: 'user', role: 'follower' })));
        }
    ...
    if (hasRemove) {
        ...
        if (rest.removeAssigneeIds?.length) {
            members.push(...rest.removeAssigneeIds.map((id) => ({ id, type: 'user', role: 'assignee' })));
        }
        if (rest.removeFollowerIds?.length) {
            members.push(...rest.removeFollowerIds.map((id) => ({ id, type: 'user', role: 'follower' })));
        }
 ...
 * 批量删除飞书任务
 */
export async function deleteTasks(taskGuids, api) {
