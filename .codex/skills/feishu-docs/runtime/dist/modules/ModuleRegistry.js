...
const USER_ONLY_MODULES = new Set(['task', 'calendar', 'member']);
/**
 * 功能模块注册中心
...
export class ModuleRegistry {
    ...
    static getEnabledModules(enabledIds, authType = 'tenant') {
        ...
        if (authType === 'tenant') {
            for (const id of USER_ONLY_MODULES)
                resolved.delete(id);
        }
        return ALL_MODULES.filter(m => resolved.has(m.id));
    }
    /** 返回所有可用模块的 ID 列表 */
    static getAllModuleIds() {
...
Object.defineProperty(ModuleRegistry, "moduleMap", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: new Map(ALL_MODULES.map(m => [m.id, m]))
});
