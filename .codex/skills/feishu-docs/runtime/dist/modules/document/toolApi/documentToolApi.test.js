...
test('createDocument ignores wikiContext with empty spaceId when folderToken is provided', async () => {
    const api = {
        createDocument: async (title, folderToken) => ({ title, folderToken }),
    };
    const result = await createDocument({ title: 'Drive Doc', folderToken: 'folder-123', wikiContext: { spaceId: '' } }, api);
...
test('createDocument still rejects when both folderToken and a valid wikiContext are provided', async () => {
    const api = {};
    await assert.rejects(createDocument({ title: 'Conflict Doc', folderToken: 'folder-123', wikiContext: { spaceId: 'space-123' } }, api), /不能同时提供 folderToken 和 wikiContext/);
});
