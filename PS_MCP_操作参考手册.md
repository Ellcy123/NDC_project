...
| 人手、背景、承托物、场景残片等外部缺陷 | 对选定版按实时能力执行适用的独立隔离路线 | 是 |
...
- 同一路线的同一原样命令最多重试一次，不在同一源上无限追加阈值、描边或参数微调；但一个命令、动作或路线失败绝不代表 Photoshop 整体能力耗尽。必须先按第 4.3 节完成适用独立路线清单与逐路实测；任何已导出的不完整 RGBA 同时按第 4.2 节保留并推进，不等待其它路线全部结束后才建立审核材料。
...
在宣告“Photoshop 无法提取”“没有可用方法”或停止依赖透明层的生产前，必须先基于当前命令目录、源图材质和任务授权建立 `route_exhaustion_log`。这里的“路线”是从当前源副本到可核验 RGBA 的端到端方法，不是某个按钮、选区或命令名称。

1. 先登记所有适用路线及状态：原生 Alpha；可直接得到透明结果的 Remove Background／已安装非生成动作；Select Subject 或其它语义选区加当前 `supported` 的蒙版、通道、删除背景或导出实现；通道／计算派生 Alpha；只针对适合硬边区域的路径；具体 Skill 已授权的确定性 Alpha；以及已为当前资产明确授权的人工路线。`requires_user`、`unavailable`、材质不适用或超出 Skill 授权的路线必须写明理由，不冒充已执行。
2. 对每条适用且端到端命令均为 `supported` 的自动路线，至少在新的未保存副本上实际执行一次，保存 command ID、status、源哈希、临时文档 ID、输出或精确失败点与视觉结论。同一原样命令失败最多重试一次，之后换下一条独立路线；不得把重复参数微调包装成新路线。
3. 某一步成功不等于整条路线成功；例如“选择主体”已有选区但无法以任何当前受支持方式落实为 Alpha，只说明这条 `selection_to_rgba` 路线在落实步骤受阻。反过来，“移除背景”已导出但仍保留矩形场景背景，是一份真实但质量失败的 RGBA：立即按第 4.2 节登记 provisional，同时继续其它尚未实测的适用路线。
4. 获得正式视觉通过的 RGBA 后可停止其余路线；否则必须完成所有适用路线的实测或不适用／不受支持证据，才能登记能力耗尽。存在任何可打开的合规非生成 RGBA 时，不得写成“完全没有输出”，也不得因此停止 provisional 下游。
5. 路线耗尽不授权界面自动化、其它生图后端或把制作劳动转交用户。Computer Use／界面操作只在当前任务另有明确授权时才可列入路线；未授权时从清单中排除并记录边界。

`route_exhaustion_log` 至少包含：`route_id`、`material_fit`、所需命令及实时状态、`attempted`、调用回执、输入／输出路径与 SHA-256、`result: PASS | PROVISIONAL | FAILED | NOT_APPLICABLE | UNSUPPORTED`、失败点和下一路线。没有该日志及逐路证据，不得把一个失败动作概括成 Photoshop 能力阻断。
...
- 副本上执行“移除背景”动作；
- 每条适用独立路线先做一次实际隔离；同一原样命令的第二次仅用于第 16 节允许的明确失败重试；
- 检查所有组件、细线、孔洞和当前角色所需阴影；
- 通过后再进入最终尺寸整理；
- 同一路线失败后不继续无限调阈值或描整圈；先按第 4.2 节保存并交付 provisional 提取包，同时按第 4.3 节继续其它适用的受支持独立路线，再按剩余额度决定是否重生空背景或原生透明版本。
...
- 回到副本初始状态；
- 同一原样命令最多重试一次；
- 再失败则换下一条适用独立路线；只有第 4.3 节路线耗尽日志完成后才可记录整体能力缺失。
...
- 任何抠图路线都要保留一次失败回退，不允许无限微调同一方法；同时必须区分“同一路线重试”与“不同路线实测”，单一路线失败不得提前宣告 Photoshop 能力耗尽。
