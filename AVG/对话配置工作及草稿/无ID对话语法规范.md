...
适用范围：采用无 ID 写作流程的新对话 MD 草稿（Talk + Expose）。Unit1 当前正式台本已保留 Unity 的九位运行 ID，不回溯改成无 ID 格式。
...
设计原则：**语法以写手书写自然、人读顺畅、机器确定解析为第一优先**。预处理器适配本语法，不让脚本现状限制语法表达力；当前 Unit1 不使用本规范。
