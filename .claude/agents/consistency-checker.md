...
2. **证词摘要一致**：Testimony.words（证词原文）与 TestimonyItem.testimony（提取摘要）信息密度是否对等？对每条证词逐项确认：
   - 时间限定词是否保留？（"大约""之前""之后""左右""当时""刚才"）
   - 地点是否保留？（具体位置不能省略或泛化）
   - 人物指代是否明确？（不能用"他/她"替代具体名字）
   - 条件/前提是否保留？（"如果""只要""除非""直到"）
   - 否定词是否保留？（"没有""不是""从未""并非"）
   - 数量/频率词是否保留？（"两次""经常""偶尔""唯一""每次"）
   丢失任何一项 → 记入冲突列表"证词摘要不一致"
3. **ID 唯一性**：
   - 对话 ID（9 位）：全局无重复
   - 证据 ID（4 位）：全局无重复
   - 证词 ID（7 位）：全局无重复
